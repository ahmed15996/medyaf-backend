...
abstract class BaseRepository implements BaseRepositoryInterface
{
    ...
    public function query($orderBy = ['column' => 'id', 'dir' => 'DESC'])
    {
        return $this->model::query()->orderBy($orderBy['column'], $orderBy['dir']);
    }
    ...
    public function searchPaginate(array $where = ['id', '>', 0], array $columns, $searchQuery, $orderBy = ['column' => 'id', 'dir' => 'DESC'])
    {
        ...
        if (count($whereArr) > 0) {
            $result = $this->model->where($whereArr)->orderBy($orderBy['column'], $orderBy['dir'])->paginate(10);
        }

        if (count($orWhereArr) > 0) {
            $result = $this->model->where($whereArr)->orWhere($orWhereArr)->orderBy($orderBy['column'], $orderBy['dir'])->paginate(10);
        }
